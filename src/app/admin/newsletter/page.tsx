...
export default function SubscribersAdminPage() {
  ...
  const load = async () => {
    try { setLoading(true); setItems(await getNewsletterSubscribers(token)); }
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">{items.length} total subscribers</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-outline-variant/10 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-surface-container-low text-slate-600 uppercase text-xs tracking-wider">
              <tr>{['Email', 'Status', 'Subscribed On'].map(h => <th key={h} className="px-4 py-3 text-left font-bold">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-outline-variant/10">
              {items.length === 0 && <tr><td colSpan={3} className="text-center py-12 text-slate-400">No subscribers yet</td></tr>}
              {items.map(item => (
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3 font-medium text-slate-800">{item.email}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${item.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
