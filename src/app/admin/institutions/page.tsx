...
export default function InstitutionsAdminPage() {
  ...
  const load = async () => {
    try { setLoading(true); const { getInstitutions } = await import('../../../lib/api'); setItems(await getInstitutions(token)); }
    finally { setLoading(false); }
  };

  const handleStatusUpdate = async (id: string, newStatus: string) => {
    try {
      const { updateInstitutionStatus } = await import('../../../lib/api');
      await updateInstitutionStatus(id, newStatus, token);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { deleteInstitution } = await import('../../../lib/api');
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Institution Inquiries</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-500 uppercase text-[10px] font-black tracking-widest border-b border-outline-variant/10">
                <tr>{['Institution Info', 'Contact Person', 'Status', 'Date', 'Actions'].map(h => <th key={h} className="px-6 py-4 text-left">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-outline-variant/5">
                {items.length === 0 && <tr><td colSpan={5} className="text-center py-20 text-slate-400 italic">No institution inquiries found</td></tr>}
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="font-bold text-slate-800 md:text-base">{item.institutionName}</div>
                      ...
                      <div className="text-slate-500 flex items-center gap-1.5 mt-0.5">
                        <span className="material-symbols-outlined text-xs">call</span> {item.contactNumber}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-700">{item.contactPerson}</div>
                    </td>
                    <td className="px-6 py-4">
                       <select 
                        value={item.status} 
                       ...
                       >
                         {['NEW', 'REVIEW', 'ONBOARDED', 'REJECTED'].map(s => (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <a href={`tel:${item.contactNumber}`} title="Call Now" className="p-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors">
