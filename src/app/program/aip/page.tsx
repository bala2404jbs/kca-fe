...
export default function AIPProgramPage() {
  ...
  useEffect(() => {
    getPageMedia('program-aip').then(setMedia).catch(console.error);
  }, []);
  return (
        ...
        <div className="lg:col-span-8 space-y-12">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {(media?.galleryImages?.length > 0 ? media.galleryImages : ["/images/programs/aip/1.jpg", "/images/programs/aip/2.jpg", "/images/programs/aip/3.jpg", "/images/programs/aip/4.jpg"]).slice(0, 4).map((img: string, idx: number) => (
                <div key={idx} className="relative h-64 rounded-xl overflow-hidden shadow-md group">
                  <Image src={getMediaUrl(img)} alt={`AIP Activity ${idx + 1}`} fill unoptimized className="object-cover transition-transform duration-500 group-hover:scale-110" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {(media?.youtubeVideoIds?.length > 0 ? media.youtubeVideoIds : ["WrleBiAGB-Q", "KBDjB75Af2Q"]).slice(0, 2).map((vid: string, idx: number) => (
                <div key={idx} className="aspect-video rounded-xl overflow-hidden shadow-md">
                  <iframe
                    width="100%"
                    height="100%"
                    src={`https://www.youtube.com/embed/${vid}`}
                    title={`AIP Video ${idx + 1}`}
                    frameBorder="0"
        ...
        <aside className="lg:col-span-4">
          <div className="sticky top-28">
            <LeadForm 
              ...
              subtitle="Register yourself for the program."
            />
          </div>
        </aside>
      </div>
